...
try:
    ...
    cursor.execute("ALTER TABLE performance ADD COLUMN kamin TEXT")
    conn.commit()
    logger.info("Колоната 'kamin' е успешно додадена во performance!")

except sqlite3.OperationalError as e:
    if "duplicate column name" in str(e):
        logger.info("Колоната 'kamin' веќе постои – нема промена")
    else:
